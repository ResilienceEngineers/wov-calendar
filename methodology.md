...
| 2026-05-23 | FM parser rewritten to read the real fm-tracker `BRIEF:FM_TABLE` (operator/site/wave/type/status/date columns, human dates, operator→country/region/industry inference); Hormuz scraper URL corrected to `hormuz-daily-brief`; active_events seeded from the live Hormuz-closure scenario | Map layers (active / FM / Hormuz) were empty; Marco asked to use the fm-tracker dataset |
| 2026-05-23 | Map markers now use click POPUPS with full detail (was hover tooltips) + Leaflet.markercluster grouping for active/FM/Hormuz layers | Marco: clicking markers showed no data; needed clustering |
| 2026-05-23 | Executive summary reworked to a prepare-what / when / act-by / why action statement; added `risk_by_month` 12-month convergence-intensity profile ("when is the riskiest time") rendered on both pages | Marco: main message must say when companies prepare for what and why; visualise the riskiest time |
